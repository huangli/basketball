...
- **多批次合并合成（2026-08-15 修复，docs/build-multi-batch/spec.md）**：选中批次 >1 时先把各批 goals 全记录逐字合并，原子写 `work/<S>/merged_goals_cli.json`（`{"session", "goals"}`；每次 build 重写；**命名不以 goals 开头**——避开 discover_batches 的 `goals*.json` 扫描防 WARNING 噪音），每个 filter 只调一次 build_highlight。缘由：输出名由 session+filter 决定、不含批次，逐批调会互相覆盖，且零球批次 "无可合成记录" exit 1 会中止整轮（立哥 2026-08-15 实测事故）。选中批次 ==1（含 --batch K）时保持逐字原路径。--dry-run 不写合并文件
- 无 --scorer/--team/--all：出全员合集（build_highlight 不带过滤参数的既有行为）
- `--scorer X` / `--team T`：单个合集（互斥，同 build_highlight 契约）；**显式过滤不做零命中预检**——零记录错误自下而上冒出，保留 typo 防呆
