...
- [x] `mark()` 内：goal 且成组且本次为新标注 → 查同组未标注成员 →
      confirm → 标 no 并跳过
- [x] 只动未标注成员，已标注一律不覆盖；重复按 J 不再弹框
- [x] 回归断言：生成 html 含确认框文案关键片段
- [x] `node --check` 生成页语法通过（node v24.15.0 已在 PATH）
- [x] 关口：ruff + pytest 全绿

## Task 3：批次 3 回放验证

- [x] 写 `work/label_speedup_replay.py`（一次性，不入库）
- [x] J 集合 = `20260722地平线/goals_20260722_3.json` 61 条精确匹配事件
      （61/61 匹配）；真球集合 = `work/20260722/goals_batch3.json` 51 条
- [x] 回放输出：跳过事件数 15 ≥ 8 PASS（= 8 条同球双 J + 7 条非球组内邻接）
- [x] 覆盖式断言：51 球每条仍有 goal 事件 PASS
      （口径修订：±2s 或同组，203946@5.9 经同组覆盖，见 review02）
