...
citymonkey 场次实锤三类废裁图（2026-08-27 三方调研发现，work/_clip_analysis/report.md）：

1. **无人框**：t480.1 c0 裁的是场边瓶子（轨迹法定位错误），无人废图竟过 CLIP 闸
2. **畸形框**：t550.0 c0 宽高比 0.09（正常站立人 0.28~0.58），检测框漂成细条，
   经"短边不足 400 等比放大"（crop_scorers.py:976）硬拉 11 倍 → 400×4456 糊图
3. **错人框**：t186.6 c1/c2、t34.1 c2、t264.4 c0 裁的是对方蓝衣球员（定位错人）

废帧污染全部下游：CLIP 聚类、照片匹配、K3 读号、确认页认人。本功能在
**裁图产出环节**加质量闸：废帧不进 crops，多裁选帧机制自然跳过废帧选次优帧。

成功标准（可执行验证）：

- 质量闸落地后，citymonkey 重跑 crop：t480.1 瓶子帧、t550.0 细条帧被拦且留痕；
  13 球已终裁样本的有效裁图不误杀（误杀率 0 容忍——宁漏拦不错杀，
  错杀直接损失认人证据）
...
# citymonkey 重跑验证（删除旧 crop 产物后重跑 people 裁图段，命令以 plan 为准）
```
...
- **幂等**：只对新跑生效；已产出的旧 scorer_candidates.json 不重判。
  citymonkey 验证靠删旧产物重跑（操作步骤写 plan）
...
- citymonkey 重跑真机验证走人工核对（t480.1/t550.0 被拦、13 球好图不误杀），
...
- [ ] citymonkey 重跑验证：t480.1/t550.0 被拦、13 球好图零误杀，归档 review
