...
**Acceptance criteria：**
- [ ] 0s→1、6.1s→31、负值/越界行为有定义（钳位到 ≥1）
- [ ] `tests/test_pipe_common.py` 新增映射用例全过
...
**Acceptance criteria：**
- [ ] 帧号钳位 [1, 帧数]；缺帧降级不崩且有 WARNING
- [ ] key 文件名安全化（`#`/`@` 等）
- [ ] 批次 3 回放：234 事件全产，降级清单零意外（51 球 3 帧全到位或合法降级）
...
**Acceptance criteria：**
- [ ] raw string 模板；注入 grp 字段（复用 assign_same_rally_groups）
- [ ] 已标事件 F 按钮禁用且展示既有标注（渲染读 localStorage 实时值）
- [ ] save 合并写复刻；页面代码不含 POSKEY 写
- [ ] 回归断言 + `node --check` 通过
...
**Acceptance criteria：**
- [ ] 切批：fid 文件名排序、--batch-size 默认 50、K 从 1 递增
- [ ] --fids → adhoc 固定命名，不占批次序号
- [ ] 产物校验器：好 JSON 跳过 / 截断重算或终止；facts 篡改 → WARNING 终止
- [ ] 混合分辨率/帧率 → WARNING 列明细并终止（spec 阶段①）
- [ ] 单文件失败记 WARNING 继续，结尾汇总失败清单并非零退出（鲁棒条）
- [ ] dry-run/日志如实标注 ② 为"全场抽 + 幂等跳过"
- [ ] dry-run 命令含全部显式参数与 --keep-clips
- [ ] 单测：切分、校验器、facts 比对、dry-run 清单（探测可注入）
...
**Acceptance criteria：**
- [ ] dry-run 对 20260722 出 6 批 × 7 阶段清单，人工对照历史命令等价
- [ ] 续跑断言：缓存齐单批 ②③ < 30s（日志时间戳）
- [ ] --fids 3 dji 小样本端到端出 adhoc 全套
- [ ] 故障注入两项（facts 篡改 / candidates 截断）行为正确
...
**Acceptance criteria：**
- [ ] todo 回填；spec-reviewer 实施复审写 review03.md
