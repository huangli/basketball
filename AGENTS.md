...
- **检测流水线**（详见方案文档）：抽帧 5fps → abdullahtarek+yolov8n 检测 → MOT 静止段+断轨重连候选 → 筐轨迹补检 → 事件合并+筐距排序 → label.html 标注页（J/P/F）→ goals.json → build_highlight.py 合成（--out 按场次注入尺寸）；**事件级 K3 判定已下线**（2026-08-01 立哥定：批次 2 实测 7 次 YES 仅 6 真 1 误、仅覆盖 6/37 球，NO 误杀真球 4 起、排尾险致漏球，排序性价比过低，批次 3 起不跑）
...
## 当前状态（2026-08-08；认人闭环完成）

- **批次 1（20260722 首批 50 视频）全链路闭环（含认人）**：17 球 → 12 个个人合集 + 2 个分队集锦（黑 3 / 白 6；便服 8 球只进个人合集）→ `output/20260722/`；`roster.json` 已生成并经立哥两轮修正（黑21 拆分 大斌/王敏龙、白22 并入小朱、蓝27 归黑队）
- **认人流程（已固化）**：crop_scorers.py（轨迹法定位+裁图+颜色分队预填）→ gen_scorer_page.py（确认页，裁图仅辅助、视频为终裁）→ 立哥确认导出 roster.json → build_highlight.py --roster/--scorer/--team 出合集；四件套在 `docs/scorer/`（spec/plan/todo/review01-03）
...
- **批次 2（100 视频）**：185 事件机器部分完成（另一会话；含审核片段 +4s 尾巴、默认全景、跨文件续接），**等立哥标注** `work/20260722/review_batch2/label.html`；批次 2 认人复用本流程（--roster-existing）
- 全量推进**暂缓**，等立哥指令；球员照片库待立哥新增（到手后可上人脸识别预填）
