...
- **半截篮队伍集锦默认 4K**（2026-08-30 立哥定，docs/build-4k/）：`video build --all` / `--team 半截篮` 时 `队伍_半截篮_进球集锦.mp4` 出 4K（16:9 场次 3840×2160，4:3 场次 2880×2160），其余产物维持 1080p；手动重出用 `video build … --4k`（产物加 `_4K` 后缀，如 `半截篮_黄立_4K_进球合集.mp4`），半截篮集锦对 `--4k` 为幂等 no-op；未认人自动模式不受 4K 规则影响，仍 1080p
...
- **检测流水线**（详见主文档）：抽帧 5fps → abdullahtarek+yolov8n 检测（顺带存筐检测入 mot 缓存）→ MOT 静止段+断轨重连候选 → 筐轨迹（**2026-08-16 起缓存优先免重复推理**，旧缓存回退逐帧补检，见 `docs/detect-hoops-cache/`）→ 事件合并+筐距排序 → label.html 标注页（J 定锚/T 机器锚/P/F，G 跳进球补锚，片段自带 2x 烘焙；疑似同回合分组与倍速控制等提效功能见 `docs/dedup-same-goal/`、`docs/label-speedup/`，人工锚点见 `docs/goal-anchor/`、`docs/label-jump-goal/`）→ goals.json → build_highlight.py 合成（--out 按场次注入尺寸）；**新场次用 `run_session.py <素材目录> --session <场次ID>` 一键串联至标注页生成（切批/断点续跑/尺寸探测注入；标注与合集合成仍手工），见 `docs/batch-speedup/`**；triage 缩略图墙已下线（2026-08-11 立哥定，与标注页功能重复，见 `docs/batch-speedup/review07.md`）；**事件级 K3 判定已下线**（2026-08-01 立哥定，对照账见主文档 §4 批次 2）
- **统一入口 CLI**：`python scripts/video.py score|people|build|photo`（PowerShell 别名 `video` 已装；任意目录可跑，自动定位仓库根）——薄封装上述链路（score=run_session 检测串联、people=认人三段链、build=合集合成+收尾自动出进球热图（暗场/分区双图，docs/heatmap/；热图仅认人后 confirmed roster 路径触发，未认人自动模式跳过）、photo=精彩照片链 rank_photos 打分抽帧裁切 → gen_photo_page 瀑布流确认页 → `--apply` 落盘 `output/<场次>/照片精选/`，四件套见 `docs/photo-select/`；批次自动发现、尺寸按 facts 换算、srcdir 记 `work/<场次>/video_cli.json`），score/people/build 四件套见 `docs/video-cli/`；**`--session` 四命令均可省（2026-08-29）**：score 缺省取素材目录 basename 且成功后写当前场次指针 `work/current_session.json`（显式 `--session` 跑 score 同样写指针），people/build/photo 缺省读指针，显式 `--session` 永远优先（people/build/photo 显式只是临时覆盖、不改写指针），指针缺失/损坏显式报错不猜场次（四件套见 `docs/default-session/`）；**立哥用操作手册 = 根目录 `使用手册.html`（2026-08-14 建，流程/快捷键/文件改名规则/FAQ 以此为准，CLI 行为变更时同步更新）**
- **文档自审（强制）**：创建或修改 `docs/` 下文档（含各功能子文件夹四件套）、`AGENTS.md`、`rules.md` 后，必须通过 Task 工具调用 `spec-reviewer` 子代理审查；有阻断问题须修订后再交付，禁止跳过
- 进球归属：个人合集需标进球者；立哥人工标注+确认页（聚类分组+名单按钮+视频终裁）。机器认人演进（docs/photo-roster/）：CLIP 照片匹配证伪（review03）→ 人脸产品化（face_match_scorers，review04）→ 全场评测采纳误指认 100% 不达标（review05，根因=错人框一阶）→ **2026-08-29 立哥定纯人工**：people ②.5 默认关（`--photo-match` 显式开可恢复）；`photos/names.json` 号码=姓名名单保留（确认页按钮出真人名，人工链基础设施非预填）
- **认人流程（已固化）**：crop_scorers.py（轨迹法定位+多裁选帧+**质量闸两道**（宽高比 0.15/1.2 先行 + yolov8n 框内复检 0.25 殿后，废帧补位、全废球 SKIP+quality_gate，docs/crop-quality/）+串人守卫+颜色分队预填；--read-numbers 多帧众数投票读号，**默认关**（v2.1 零 token，显式开））→ propagate_scorers.py（**轨迹传播，2026-09-02 起在 people 链中**：已确认归属沿文件内人轨迹预填同轨迹球，mixed 轨迹/NOGOAL 不传播，标定挂起待新场次）→（可选）cluster_scorers.py（聚类仅分组预填不终裁，定稿 **CLIP `--linkage complete --threshold 0.15`**；OSNet 后端备用不推荐）→ gen_scorer_page.py（确认页，--track-links 传播预填+--clusters 簇级选人+逐球覆盖，--players-file 注入名单或自动读 photos/names.json，裁图仅辅助、视频为终裁）→ 立哥确认导出 roster.json → build_highlight.py --roster/--scorer/--team 出合集；参数与标定细节见 `docs/scorer/`、`docs/scorer-cluster/`、`docs/scorer-reid/`、`docs/scorer-propagate/`
