...
- **统一入口 CLI**：`python scripts/video.py score|people|build`（PowerShell 别名 `video` 已装；任意目录可跑，自动定位仓库根）——薄封装上述链路（score=run_session 检测串联、people=认人三段链、build=合集合成；批次自动发现、尺寸按 facts 换算、srcdir 记 `work/<场次>/video_cli.json`），四件套见 `docs/video-cli/`
