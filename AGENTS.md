...
- **认人流程（已固化）**：crop_scorers.py（轨迹法定位+多裁选帧+**质量闸两道**（宽高比 0.15/1.2 先行 + yolov8n 框内复检 0.25 殿后，废帧补位、全废球 SKIP+quality_gate，docs/crop-quality/）+串人守卫+颜色分队预填；--read-numbers 多帧众数投票读号，**默认关**（v2.1 零 token，显式开））→（可选）cluster_scorers.py（聚类仅分组预填不终裁，定稿 **CLIP `--linkage complete --threshold 0.15`**；OSNet 后端备用不推荐）→ gen_scorer_page.py（确认页，--clusters 簇级选人+逐球覆盖，--players-file 注入名单或自动读 photos/names.json，裁图仅辅助、视频为终裁）→ 立哥确认导出 roster.json → build_highlight.py --roster/--scorer/--team 出合集；参数与标定细节见 `docs/scorer/`、`docs/scorer-cluster/`、`docs/scorer-reid/`
