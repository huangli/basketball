...
- **批次 1（20260722 首批 50 视频）端到端闭环**：365 候选 → 113 事件 → 人工标注 20 球 + 1 训练球 → 去重（190354 同球拆两次）+ 剔误标（190330 传球、190428 弹出）→ **17 球合集已出** `output/20260722/个人_全员_进球合集.mp4`（1920×1080 50fps）；补标 16 事件（覆盖 20 个未覆盖时刻）0 进球，候选级召回账目闭环
...
- 下批（全量 300）五改进：筐距排序写入 events_index、事件聚类改时间+空间、事件级 K3 为默认判定协议、自动剔除必配补标机制、长事件锚点取事件末成员（max-conf 锚点在长事件切错时段，批次 1 有 3 例）
